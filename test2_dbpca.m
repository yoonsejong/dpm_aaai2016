...
THRESHc = 1e-4;     % threshold for centralized methods (PPCA, VBPCA)
...
MissRate = 10;       % missing rate in percentage
if MissRate > 0
    seq = randperm(D * N);
...
[cm1, cm2] = expr_run_cppca(X, M, THRESHc, objfreq_c, m_init_c, false, run_bayesian);
...
[cm3, cm4] = expr_run_dppca(X, M, V, E, ETA, THRESHd, objfreq_d, m_init_d, false, run_bayesian);
