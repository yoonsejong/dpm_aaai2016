...
defaultType = 'cppca';
expectedTypes = {'cppca', 'dppca', 'sfm_c', 'sfm_d'};

[D, N] = size(X);
defaultJ = 1;
defaultSampleAssignVec = ones(N, 1);

addParameter(p,'ModelType',defaultType, ...
    @(x) any(validatestring(x,expectedTypes)));
addParameter(p,'NumberOfNodes',defaultJ,@isnumeric);
addParameter(p,'SampleAssignVec',defaultSampleAssignVec);
...
elseif strcmp(p.Results.ModelType, 'sfm_d')
    ...
    for j = 1 : J
        ...
        model_init.W{j} = ...
            [ X( past_frames + 1 : past_frames + 2, : ) + ones(2,N)/VARI; ones(1,N)]';
        % cancel out if missing
        NcI = isnan(X(past_frames + 1, :));
        model_init.W{j}(NcI,:) = [ ones(2,sum(NcI))/VARI; ones(1,sum(NcI)) ]';
        % MU should be initialized as zero
        model_init.MU{j} = zeros(N, 1);
        % VAR is just a small number
        model_init.VAR{j} = rand(1) + D_PARAM;    
    end
end
